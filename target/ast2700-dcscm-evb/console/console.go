...
func ehciEnum(b *strings.Builder, c *ehci.Controller, port ehci.Port) {
	...
	if err != nil {
		...
		fmt.Fprintf(b, "  CTRLDSSEG = %#010x  QHphys=%#012x (low32=%#010x)\n", d.CtrlDSSeg, d.QHPhys, uint32(d.QHPhys))
		fmt.Fprintf(b, "  ASYNCADDR imm=%#010x (after write) -> %#010x (after run)\n", d.AsyncAddrImm, d.AsyncAddr)
		fmt.Fprintf(b, "  QH in DRAM: link=%#010x epchar=%#010x  curQTD=%#010x overlayTok=%#010x\n", d.QHLinkMem, d.QHCharMem, d.QHCurQTD, d.QHToken)
